...
ノートのエディタとタグ・検索結果では、リンクをCodiconsの `link-external` アイコンとリンク先で表示します。`[](https://example.com)`、`<https://example.com>`、`[[TRIP]]`、`[名前](https://example.com)`、裸のURLに対応します。`[旅行予定](旅行)` のように表示名がある場合は「アイコン＋旅行予定」を表示し、表示名が空の場合はリンク先を表示します。コード内と画像記法は置き換えません。エディタでリンクにカーソルや選択範囲を重ねると元の記法に戻り、編集できます。保存されるMarkdownは変更しません。`[[TRIP]]` は現在のノートを基準とした `TRIP/index.md` を指します（ディレクトリが存在する場合）。タグ・検索結果のクリックは従来どおり元のノートの該当位置に移動します。

リンクのアイコンは `fnote.linkMark` で指定できます。既定値は `"$(link-external)"` です。たとえば `"fnote.linkMark": "$(globe)"` または `"fnote.linkMark": "🔗"` を設定すると両画面に反映されます。空文字 `""` はアイコンを非表示にします。同梱されていないCodicon名は既定のアイコンに戻ります。

`[旅行予定](../旅行/index.md)` のようなファイル名を含む相対リンクにも対応します。既存のファイルは直接開き、ディレクトリの場合はその中の `index.md` を開きます。
