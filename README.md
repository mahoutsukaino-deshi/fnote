...
`@land-cruiser`・`@rav4` があると、タグ一覧では `toyota` の配下に表示します。親タグを選ぶと子孫のタグを持つノートと該当行をまとめて表示します。使用されていないタグは表示しません。複数段の定義も可能です。複数の親を定義した場合は先の定義を優先し、循環する定義は無視します。設定は一覧の分類に適用され、本文のタグ名やタグの色・印の優先順位は変更しません。
