...
選ばれた定義に文字色・背景色がなければ、それぞれ `fnote.tagColor`・`fnote.tagBackgroundColor` を使用します。子タグに独自の定義がある場合、未指定の印・文字色・背景色を親タグの定義から補うことはありません。`backgroundColor: ""` は既定の背景色を無効にします。
